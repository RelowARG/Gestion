...
router.get('/', async (req, res) => {
    try {
      // Fetch all products with relevant columns
      const [rows, fields] = await req.db.execute('SELECT id, codigo, Descripcion, eti_x_rollo, costo_x_1000, costo_x_rollo, precio, banda, material, Buje FROM Productos ORDER BY codigo ASC');
      res.json(rows);
    } catch (error) {
  ...
  router.post('/', async (req, res) => {
    ...
    const { codigo, Descripcion, eti_x_rollo, costo_x_1000, costo_x_rollo, precio, banda, material, Buje } = req.body;
    ...
    const sql = `
      INSERT INTO Productos (
        codigo, Descripcion, eti_x_rollo, costo_x_1000, costo_x_rollo, precio,
        banda, material, Buje
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;

    // Ensure numerical values are parsed correctly or set to null
    const values = [
      codigo,
      Descripcion,
  ...
  router.get('/:id', async (req, res) => {
    ...
    const sql = `
      SELECT
        id, codigo, Descripcion, eti_x_rollo, costo_x_1000, costo_x_rollo, precio,
        banda, material, Buje
      FROM Productos
      WHERE id = ?`;
  ...
  router.put('/:id', async (req, res) => {
    ...
    // Basic validation
     ...
     try {
        ...
        // 5. Proceed with updating the product in the Productos table
        ...
        // Prepare values for the update query, using the NEW parsed costs
        const values = [
          codigo,
          Descripcion,
          eti_x_rollo !== '' && eti_x_rollo !== null ? parseFloat(eti_x_rollo) : null,
